...
const TYPE_CONFIG = {
  timetable: {
    label: 'Changement EDT',
    filterLabel: 'Emploi du temps',
...
function buildSubtitle(notifs) {
  const unread = notifs.filter(n => !n.read).length;
  if (unread === 0) return 'Tout est à jour';
  const types = [...new Set(notifs.filter(n => !n.read).map(n => TYPE_CONFIG[n.type]?.filterLabel?.toLowerCase()))].slice(0, 3);
  return `${unread} non-lue${unread > 1 ? 's' : ''} · ${types.join(', ')}`;
}
      ...
      <div className="flex gap-4 items-start">
        ...
        <aside className="hidden md:block w-48 shrink-0 border border-[var(--color-border)] rounded-xl bg-[var(--color-bg-elev)] overflow-hidden">
          <div className="px-4 py-3 border-b border-[var(--color-border)]">
            <span className="text-xs font-semibold text-[var(--color-text-muted)] tracking-wide">Filtrer</span>
          </div>
          <ul className="py-1">
            {FILTERS.map(f => {
