  ...
  return (
    <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-elev)] mt-6 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 bg-[var(--color-surface)] border-b border-[var(--color-border)]">
        ...
        <span className="text-sm font-semibold text-[var(--color-text)]">{translate('perfByCourse')}</span>
        <span className="text-xs text-[var(--color-text-muted)] ml-1">({coursePerformance.length})</span>
      </div>
              ...
              <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6">
                <div className="sm:col-span-1 min-w-0">
                  ...
                  <div className="text-xs text-[var(--color-text-muted)] mt-0.5">
                    {courseId} · {studentsCount} {translate(studentsCount === 1 ? 'attStudentSingular' : 'attStudentPlural', { n: '' }).trim()}
                  </div>
                </div>

                <div className="flex flex-col gap-0.5">
                  <span className="text-xs text-[var(--color-text-muted)]">{translate('perfAverage')}</span>
                  <span className={`text-lg font-bold ${avgTextColor}`}>{avg !== null ? `${avg}/20` : '—'}</span>
                </div>

                <div className="flex flex-col gap-0.5">
                  <span className="text-xs text-[var(--color-text-muted)]">{translate('perfPassRate')}</span>
