...
export default function DashboardSidebar() {
  ...
  const isActive = (item) => {
    ...
    // Special handling for the single student path (/dashboard/student): match on query tab
  ...
  const getSidebarConfig = () => {
    ...
    if (role === 'student') {
      // Single path only: /dashboard/student + ?tab=... so only the main content area changes (no full page load, layout/shell stays mounted).
      // This matches the requirement for one path per role dashboard with content swapping inside the main area.
      mainItems = [
      ...
      toolsItems = [
        { label: ariaLabel, href: '/dashboard/assistant', icon: Sparkles }, // fake that references real /aria/ while providing dashboard shell
