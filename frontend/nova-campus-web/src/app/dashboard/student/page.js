...
/**
 * Single-path student dashboard: /dashboard/student
 * Only the content inside the main area of the dashboard layout changes when switching sections.
 * The layout (header + sidebar) stays mounted — no full page loads.
 * Uses ?tab=... query for bookmarkable/deep-linkable sections (client-side switch).
 * All styling uses current CSS tokens from globals.css for fidelity (no mockup hex copies).
 * This is the final functional version (real interactions, realistic data).
 * Backend data (via services + updated seeding) will replace mocks later.
 */
...
export default function StudentDashboard() {
  ...
  // Local state for functional interactions (final UI behavior; will sync with real backend data later)
  const [gradesData, setGradesData] = useState([
    { course: 'Introduction au Business (COM101)', moyenne: '14.2', evaluations: [
      { date: '12/10', type: 'Quiz 1', coef: 1, note: '15/20' },
      { date: '06/11', type: 'Partiel intermédiaire', coef: 2, note: '13/20' },
      { date: '22/11', type: 'Cas d\'entreprise', coef: 1, note: '16/20' },
      { date: '28/11', type: 'Présentation orale', coef: 2, note: '13/20' },
    ]},
    { course: 'Économie Internationale (ECO101)', moyenne: '12.5', evaluations: [] },
    { course: 'Anglais professionnel', moyenne: '15.8', evaluations: [] },
    { course: 'Projet entrepreneurial', moyenne: '14.0', evaluations: [] },
  ]);
  ...
  const markNotifRead = (id) => {
    ...
    // Sync with live context when viewing the notifications section
  ...
  const justifyAbsence = (id) => {
    ...
    alert('Justificatif soumis (simulation — sera connecté au backend plus tard).');
  };
  ...
  const payEcheance = (index) => {
    ...
    setPayments(updated);
    alert('Paiement effectué (simulation). Solde mis à jour.');
  };

  // Realistic mock data for the test student (will be replaced by seeded service data)
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  {['08:00-10:00', '10:00-12:00', '14:00-16:00', '16:00-18:00'].map((slot, i) => (
                    <tr key={i}>
                      ...
                      <td className="p-3">{i === 0 ? 'Introduction au Business (COM101) — Amphi Commerce A' : i === 2 ? 'Économie Internationale — Salle 204' : '-'}</td>
                      <td className="p-3">{i === 1 ? 'Anglais Pro — Salle 12' : '-'}</td>
                      <td className="p-3">{i === 0 ? 'Projet Entrepreneurial' : i === 3 ? 'Marketing Stratégique' : '-'}</td>
                      <td className="p-3">{i === 2 ? 'Introduction au Business (TD)' : '-'}</td>
                      <td className="p-3">{i === 1 ? 'Économie Internationale (TD)' : '-'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="mt-3 text-xs text-[var(--color-text-muted)]">Source : base de données planning (sera synchronisé avec le service scheduling).</p>
          </div>
        );
      ...
      case 'grades':
        return (
          <div>
            ...
            {gradesData.map((g, idx) => (
              <div key={idx} className="mb-4 border border-[var(--color-border)] rounded-lg p-4 bg-[var(--color-bg-elev)]">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="text-sm">Votre moyenne : <span className="font-semibold">{g.moyenne}</span></div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="text-[var(--color-text-muted)]">
                        ...
                        <th className="text-left py-1">Évaluation</th>
                        <th className="text-left py-1">Coef.</th>
                        <th className="text-left py-1">Note</th>
                      </tr>
                    </thead>
                    <tbody>
                      {g.evaluations.map((e, i) => (
                        <tr key={i} className="border-t border-[var(--color-border)]">
                          <td className="py-1">{e.date}</td>
                          <td className="py-1">{e.type}</td>
                          <td className="py-1">{e.coef}</td>
                          <td className="py-1 font-medium">{e.note}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <button onClick={() => alert('Export bulletin (simulation)')} className="mt-2 px-3 py-1.5 text-sm rounded bg-[var(--color-primary)] text-[var(--color-on-primary)]">Exporter bulletin PDF</button>
          </div>
        );
      ...
      case 'absences':
        return (
          <div>
                ...
                <tbody className="divide-y divide-[var(--color-border)]">
                  {absences.map(a => (
                    <tr key={a.id}>
                      ...
                      <td className="p-3">
                        <span className={`px-2 py-0.5 rounded text-xs ${a.status.includes('Non') ? 'bg-[var(--color-error)]/10 text-[var(--color-error)]' : 'bg-[var(--color-success)]/10 text-[var(--color-success)]'}`}>
                          {a.status}
                        </span>
                      </td>
                      <td className="p-3 text-right">
                        {a.status.includes('Non') && (
                          <button onClick={() => justifyAbsence(a.id)} className="text-sm px-3 py-1 rounded border border-[var(--color-border)] hover:bg-[var(--color-surface-hover)]">Justifier</button>
                        )}
                      </td>
                    </tr>
                  ))}
            ...
            <p className="mt-2 text-xs text-[var(--color-text-muted)]">Déposer un justificatif via le bouton ou depuis la page Notifications / Absences.</p>
          </div>
        );
        ...
        return (
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {['S1 2023-24 (En cours)', 'S2 2024', 'S3 2024-25', 'S4 2025', 'S5 2025-26', 'S6 2026'].map((s, i) => (
                <div key={i} className={`px-3 py-1 rounded text-sm border ${i === 0 ? 'bg-[var(--color-primary)] text-[var(--color-on-primary)] border-[var(--color-primary)]' : 'border-[var(--color-border)]'}`}>{s}</div>
              ))}
            </div>
            ...
            <table className="w-full text-sm border border-[var(--color-border)] rounded-lg overflow-hidden">
              <thead className="bg-[var(--color-surface)]">
                <tr>
                  ...
                  <th className="p-3 text-left">Note finale</th>
                  <th className="p-3 text-left">Statut</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                <tr><td className="p-3">Introduction au Business</td><td className="p-3">S1</td><td className="p-3">96%</td><td className="p-3">-</td><td className="p-3">En cours</td></tr>
                <tr><td className="p-3">Économie Internationale</td><td className="p-3">S1</td><td className="p-3">91%</td><td className="p-3">-</td><td className="p-3">En cours</td></tr>
              </tbody>
            </table>
          </div>
        );
            ...
            <div className="border border-[var(--color-border)] rounded-lg">
              {payments.map((p, idx) => (
                <div key={idx} className="flex items-center justify-between p-3 border-b last:border-b-0">
                  <div>
                    <div>{p.echeance} — {p.desc}</div>
                    <div className="text-sm text-[var(--color-text-muted)]">{p.montant}</div>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className={`text-sm px-2 py-0.5 rounded ${p.status === 'Payé' ? 'bg-[var(--color-success)]/10 text-[var(--color-success)]' : 'bg-[var(--color-error)]/10 text-[var(--color-error)]'}`}>{p.status}</span>
                    {p.status === 'À payer' && <button onClick={() => payEcheance(idx)} className="px-3 py-1 text-sm bg-[var(--color-primary)] text-[var(--color-on-primary)] rounded">Payer maintenant</button>}
                  </div>
                </div>
              ))}
            </div>
              ...
              {notifs.map(n => (
                <div key={n.id} className={`p-3 rounded border ${n.read ? 'border-[var(--color-border)] bg-[var(--color-bg-elev)]' : 'border-[var(--color-primary)] bg-[var(--color-primary-soft)]'}`}>
                  <div className="flex justify-between">
                    ...
                    {!n.read && <button onClick={() => markNotifRead(n.id)} className="text-sm text-[var(--color-primary)]">Marquer lu</button>}
                  </div>
                </div>
              ))}
            </div>
            <p className="mt-3 text-xs text-[var(--color-text-muted)]">Les notifications sont mises à jour en live via le contexte (pas de rechargement de page nécessaire).</p>
          </div>
        );
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elev)] p-4">
                ...
                <div className="text-3xl font-semibold mt-1">14,2<span className="text-base align-super">/20</span></div>
              </div>
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elev)] p-4">
                <div className="text-xs text-[var(--color-text-muted)]">TAUX DE PRÉSENCE</div>
                <div className="text-3xl font-semibold mt-1 text-green-600">96%</div>
              ...
              <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elev)] p-4">
                ...
                <div className="text-3xl font-semibold mt-1">11<span className="text-base align-super">/30</span></div>
              </div>
            </div>

            <div className="text-sm text-[var(--color-text-muted)]">
              Prochain cours dans 42 min • Introduction au Business (COM101) — Amphi Commerce A.<br />
              Cette semaine : 4 cours, 1 évaluation, 2 tâches en attente.<br />
              (Contenu final — données mockées en attendant le seeding complet des services.)
            </div>
          </div>
        );
    }
  ...
  return (
    <div>
      <div className="mb-4 text-sm text-[var(--color-text-muted)]">
        Novacampus / Espace étudiant / {TABS[currentTab] || 'Tableau de bord'}
      </div>
      {renderSection()}
    </div>
  );
